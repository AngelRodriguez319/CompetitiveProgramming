#include <bits/stdc++.h>
using namespace std;

// Author: Angel Gabriel Rodriguez
// Date: April / 17 / 2021
// https://www.hackerrank.com/contests/june-world-codesprint/challenges/equal-stacks/problem

string ltrim(const string &);
string rtrim(const string &);
vector<string> split(const string &);

/*
 * Complete the 'equalStacks' function below.
 *
 * The function is expected to return an INTEGER.
 * The function accepts following parameters:
 *  1. INTEGER_ARRAY h1
 *  2. INTEGER_ARRAY h2
 *  3. INTEGER_ARRAY h3
 */

int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    if(h1.size() == 0 || h2.size() == 0 || h3.size() == 0){
      return 0;
    }
    int aux = 0, aux1 = 0;   
    vector<int> H1, H2, H3;

    for(auto x = h1.rbegin(); x != h1.rend(); x++){
      aux += *x;
      H1.push_back(aux);
    }
    aux = 0;
    for(auto x = h2.rbegin(); x != h2.rend(); x++){
       aux += *x;
        H2.push_back(aux);
    }
    aux = 0;
    for(auto x = h3.rbegin(); x != h3.rend(); x++){
       aux += *x;
        H3.push_back(aux);
    }
    aux = 0;
    if(H1.size() <= H2.size() && H1.size() <= H3.size()){
        // H1 min
        for(auto x = H1.rbegin(); x != H1.rend(); x++){
            aux = count(H2.rbegin(), H2.rend(), *x);
            aux1 = count(H3.rbegin(), H3.rend(), *x);
            if(aux > 0 && aux1 > 0){
                return *x;
            }
        }
    }else if(H2.size() <= H1.size() && H2.size() <= H3.size()){
        // H2 min
        for(auto x = H2.rbegin(); x != H2.rend(); x++){
            aux = count(H1.rbegin(), H1.rend(), *x);
            aux1 = count(H3.rbegin(), H3.rend(), *x);
            if(aux > 0 && aux1 > 0){
                return *x;
            }
        }
    }else{
        //H3 min
        for(auto x = H3.rbegin(); x != H3.rend(); x++){
            aux = count(H2.rbegin(), H2.rend(), *x);
            aux1 = count(H1.rbegin(), H1.rend(), *x);
            if(aux > 0 && aux1 > 0){
                return *x;
            }
        }
    }
    return 0;
}

int main()
{
    ofstream fout(getenv("OUTPUT_PATH"));

    string first_multiple_input_temp;
    getline(cin, first_multiple_input_temp);

    vector<string> first_multiple_input = split(rtrim(first_multiple_input_temp));

    int n1 = stoi(first_multiple_input[0]);

    int n2 = stoi(first_multiple_input[1]);

    int n3 = stoi(first_multiple_input[2]);

    string h1_temp_temp;
    getline(cin, h1_temp_temp);

    vector<string> h1_temp = split(rtrim(h1_temp_temp));

    vector<int> h1(n1);

    for (int i = 0; i < n1; i++) {
        int h1_item = stoi(h1_temp[i]);

        h1[i] = h1_item;
    }

    string h2_temp_temp;
    getline(cin, h2_temp_temp);

    vector<string> h2_temp = split(rtrim(h2_temp_temp));

    vector<int> h2(n2);

    for (int i = 0; i < n2; i++) {
        int h2_item = stoi(h2_temp[i]);

        h2[i] = h2_item;
    }

    string h3_temp_temp;
    getline(cin, h3_temp_temp);

    vector<string> h3_temp = split(rtrim(h3_temp_temp));

    vector<int> h3(n3);

    for (int i = 0; i < n3; i++) {
        int h3_item = stoi(h3_temp[i]);

        h3[i] = h3_item;
    }

    int result = equalStacks(h1, h2, h3);

    fout << result << "\n";

    fout.close();

    return 0;
}

string ltrim(const string &str) {
    string s(str);

    s.erase(
        s.begin(),
        find_if(s.begin(), s.end(), not1(ptr_fun<int, int>(isspace)))
    );

    return s;
}

string rtrim(const string &str) {
    string s(str);

    s.erase(
        find_if(s.rbegin(), s.rend(), not1(ptr_fun<int, int>(isspace))).base(),
        s.end()
    );

    return s;
}

vector<string> split(const string &str) {
    vector<string> tokens;

    string::size_type start = 0;
    string::size_type end = 0;

    while ((end = str.find(" ", start)) != string::npos) {
        tokens.push_back(str.substr(start, end - start));

        start = end + 1;
    }

    tokens.push_back(str.substr(start));

    return tokens;
}